# algos/improvements/train_improved.py
...
class ImprovedTrainConfig:
    ...
    lambda_K_smooth: float = 0.0    # gain smoothness penalty strength


def train_improved_kalmannet_linear(
    model,
    sysmodel,
    X_train: np.ndarray,
    Y_train: np.ndarray,
    ...
    # Not strictly needed anymore if NNBuild attached them,
    # but harmless and keeps things robust.
    model.F_mat = sysmodel.F.to(device)
    model.H_mat = sysmodel.H.to(device)
    ...
    for ep in range(cfg.epochs):
        ...
        for Xb, Yb in dl:
            ...
            if cfg.x0_mode == "true":
                ...
                x0_hat = torch.zeros((B, m, 1), dtype=torch.float32, device=device)
            else:
            ...
            for t in range(T):
                ...
                if cfg.lambda_K_smooth > 0.0:
                    Kt = model.KGain  # [B,m,n]
                    if K_prev is not None:
                        # Fully differentiable smoothness penalty
                        K_smooth = K_smooth + torch.mean((Kt - K_prev) ** 2)

                    # If you ever see memory issues, replace with:
                    # K_prev = Kt.detach()
                    K_prev = Kt
